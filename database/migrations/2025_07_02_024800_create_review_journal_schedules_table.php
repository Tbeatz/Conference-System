...
return new class extends Migration
{
    ...
    {
        if (!Schema::hasTable('review_journal_schedules')) {
            Schema::create('review_journal_schedules', function (Blueprint $table) {
                $table->id();

                // FK to journal_submissions
                $table->unsignedBigInteger('journal_submission_id');
                $table->foreign('journal_submission_id')->references('id')->on('journal_submissions')->onDelete('cascade');
                ...
                // review deadline or scheduled meeting date
